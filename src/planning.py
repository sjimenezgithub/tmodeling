...
def VAL_computation_state_trajectory(domain_filename, problem_filename, plan_filename):
    ...
    # Running VAL
    domain_filename=domain_filename.replace("-empty-constraints","")
    cmd = "rm " + config.VAL_OUT + ";"+config.VAL_PATH+"/validate -t 0.000001 -a -v " + domain_filename + " " + problem_filename + " " + plan_filename + " > " + config.VAL_OUT
    print("\n\nExecuting... " + cmd)
    os.system(cmd)
