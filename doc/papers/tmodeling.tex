...
\label{sec:learning}
This section details our approach for building temporal planning models from a {\em classical planning model} and a {\em observation of plan executions}.
...
\begin{itemize}
\item $P=\tup{F,A,I,G}$ is a {\em classical planning} problem.
...
A {\em solution} to a learning task $\Lambda=\tup{P,obs(\tau)}$ is a temporal action model $\mathcal{M}$ that {\em explains} the input observation $obs(\tau)$.

\subsection{The space of temporal action models}
We assume that the durative actions $a\in A$ of a temporal planning problem $P$ are instantiated from given action schemas, as in PDDL. The space of possible models for an {\em action schema} $\xi$ of temporal actions is then given by $\Psi$, the set of {\em predicates} that shape the propositional state variables, and the list of {\em parameters} $pars(\xi)$ of the corresponding schema.
...
\begin{enumerate}
\item {\em Syntactic constraints}. We require $del_s(\xi)\subseteq cond_s(\xi)$, $del_s(\xi)\cap add_s(\xi)=\emptyset$ and $cond_s(\xi)\cap add_s(\xi)=\emptyset$ as well as the equivalent constraints for the {\em at end} time-stamp (similar syntactic constraints are usually followed when defining \strips\ action models). Considering exclusively these syntactic constraints, the size of the space of possible {\em durative} schemata is given by $2^{5\times|{\mathcal I}_{\Psi,\xi}|}$. 
...
Given a $\Lambda=\tup{P,obs(\tau)}$ task for building a {\em temporal planning} model, then we define a compilation that outputs the following $\tup{X,D,C}$ CSP:

The set of {\bf variables} $X$ comprises:
\begin{itemize}
\item For each action $a$ observed in $obs(\tau)$:
\begin{itemize}
...
\item $support_{f,a_i}$ indicates the action supporter of the fluent $f$ for action $a_i$ in the observed plan. The domain of this variable is the set of actions.
