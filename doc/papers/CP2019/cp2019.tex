...
%Learning models from military operations can be very valuable for emergency management, transportation models can be applied to traffic management decision making scenarios, etc.
...
%Given $F$, a set of {\em fluents} (propositional variables), a literal $l$ is a valuation of a fluent $f\in F$; i.e. either $l=f$ or $l=\neg f$. A set of literals $L$ represents a partial assignment of values to fluents (without loss of generality, we assume that $L$ does not contain conflicting values). We use $\mathcal{L}(F)$ to denote the set of all literal sets on $F$; i.e. all possible $L$. A state $s$ is a full assignment of values to fluents, $|s|=|F|$.
