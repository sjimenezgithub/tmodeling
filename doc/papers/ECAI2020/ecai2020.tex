...
\begin{itemize}
\item $\tup{F,I,G,A?}$ is a {\em temporal planning problem} in which actions in $A?$ are partially specified (we do not know the exact structure in terms of distribution of conditions/effects nor the duration). In the worst case, we only know the vocabulary of the symbols that can appear in the {\em conditions} and/or {\em effects} of the actions but we can also assume that prior knowledge is available bounding this vocabulary for a given action schema.
