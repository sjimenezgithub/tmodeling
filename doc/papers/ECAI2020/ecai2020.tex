...
\begin{itemize}
\item $\tup{F,I,G,A?}$ is a temporal planning problem where actions in $A?$ are {\em partially specified}. This means that the exact conditions/effects, their temporal annotation and/or the duration of actions are unknown but that the actions {\em header} (i.e., the {\em  name} and {\em parameters}) is known. %In the worst case, we only know the vocabulary of the symbols that can appear in the conditions/effects of the actions, which means having only the set of candidate conditions/effects.
...
\item $C$ is a set of {\em constraints} that reflects domain-specific expert knowledge. In this work these constraint are {\em mutually-exclusive} (mutex) constraints that allow us to deduce new observations and prune inconsistent action models. Figure~\ref{fig:example-statecs} show an example of a set of mutex constraints for the {\em driverlog} domain. 
\end{itemize}
...
The previous CSP model provides no motivation for learning preconditions that are not deleteb by an action when explaining the observations. This is a problem when the precondition of actions incldue {\em static predicates} because there are not motivation for learning this kind of preconditions~\cite{gregory2015domain}. In this work we address this issue by producing plans that are consist with the input knowledge but preferring the ones that as well provides an {\em explanation} of the observations that is as {\em tight} as possible.
